...
def page_competition_hub():
    ...
    # === Deduplicate stats (merge admin + player uploads per match) ===
    agg_cols = [
    ...
    if not df.empty and "match_id" in df.columns and "player_id" in df.columns:
        merge_strategy = {col: "max" for col in agg_cols}
        df = (
            df.groupby(["match_id", "player_id", "player_name", "position"], as_index=False)
            .agg(merge_strategy)
        )
